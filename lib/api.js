...
class Librus {
	...
	authorize(login, pass) {
		...
		return caller
			...
			.then(() => {
				return caller.postForm("https://api.librus.pl/OAuth/Authorization?client_id=46", {
					action: "login",
					login: login,
					pass: pass,
				});
			...
			.then(() => {
				return caller.get("https://api.librus.pl/OAuth/Authorization/2FA?client_id=46").then(() => {
	...
	_request(method, apiFunction, data, blank) {
		/** Make request */
		const target = apiFunction.startsWith("https://") ? apiFunction : config.page_url + "/" + apiFunction;
		return this.caller
			.request({
				method,
				url: target,
	...
	static tableValues(table) {
		return _.chain()
			.map(cheerio(table).find("tr"), row => {
				return [cheerio(row).children(0).trim(), cheerio(row).children(1).trim()];
			})
			.zipObject()
			.value();
	}
