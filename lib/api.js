...
class Librus {
  ...
  async authorize(login, pass) {
    ...
    const loginUrl = this._safeAuthorizationUrl(
      requireLocation(r2, authorizationUrl)
    );
  ...
  async _followAuthorizationChain(value, referer) {
    ...
    for (let hop = 0; hop < 10; hop += 1) {
      ...
      referer = nextUrl;
      nextUrl = this._safeAuthorizationUrl(
...
function requireLocation(response, base) {
  const location = response.headers?.location;
  if (!location) {
    throw new Error("Librus authorization response had no redirect location.");
  }
