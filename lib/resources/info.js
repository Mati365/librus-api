...
module.exports = class Info extends Resource {
	...
	getNotifications() {
		return this.api
			._mapper("uczen_index", "#graphic-menu ul li", ($, element) => {
				return $(element).text().replace(/\D+/g, "").trim().length ? $(element).text().replace(/\D+/g, "").trim() : "0";
			})
			.then(array => {
				return _.zipObject(
					["grades", "absence", "inbox", "announcements", "calendar", "homework"],
	...
	getGrade(gradeId) {
		let parser = ($, table) => {
			let keys = ["grade", "category", "date", "teacher", "lesson", "inAverage", "multiplier", "user", "comment"];
		...
		return this.api._singleMapper(`przegladaj_oceny/szczegoly/${gradeId}`, ".container-background table.decorated.medium.center tbody", parser);
	}
	/**
	...
	getPointGrade(gradeId) {
		...
		return this.api._singleMapper(`przegladaj_oceny_punktowe/szczegoly/${gradeId}`, ".container-background table.decorated.medium.center", parser);
	}
	/**
	 * Get lucky number
