...
module.exports = class Calendar extends Resource {
  ...
  getCalendar(month, year) {
    ...
    // Avoid carrying the current day into a shorter requested month/year.
  ...
  getTimetable(from, to) {
    ...
    let parser = ($, row) => {
      ...
      // Parse a single lesson ".text" div into subject/teacher/room
      let parseLesson = (textDiv) => {
        ...
        let parts = (textDiv.html() || "").split(/<br\s*\/?>/);
        if (parts.length > 1) {
          ...
          teacher = match ? match[1].trim() : teacherRoomText;
          room = match ? match[2].trim() : "";
        }
      ...
      lessonCells.each((index, cell) => {
        ...
        let lessons = $cell
          ...
          .map((_, el) => ({
            lesson: parseLesson($(el)),
            // Struck-through titles are cancelled or moved lessons
            struck: $(el).closest("s").length > 0
          }))
          .get();
        ...
        // Info label shown by Librus, e.g. "odwołane", "zastępstwo"
        let flag = $cell
          .find(".plan-lekcji-info")
          .map((_, el) => $(el).text().trim())
          .get()
          .filter(Boolean)
          .join(", ") || null;

        let active = lessons.find((entry) => !entry.struck);
        let struck = lessons.find((entry) => entry.struck);

        // Keep cancelled lessons flagged, so the consumer decides what to show
        list.push(Object.assign({}, (active || struck).lesson, {
          cancelled: !active,
          flag: flag,
          original: active && struck ? struck.lesson : null
        }));
      });
