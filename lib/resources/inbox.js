...
module.exports = class Inbox extends Resource {
    ...
    return this.api._singleMapper(
      ...
      , ($, row) => {
        let table = $(row).find("table:nth-child(2)")
          , header = this.api._mapTableValues(
              table
        ...
        return {
          ...
          , read: $(row).find("td.left:last-of-type").makeBoolean()
        };
      }
    );
  }
  ...
  listReceivers(group) {
    return this.api._mapper(
      ...
      , ($, row) => {
        return {
            id: $(row).find("input[name='DoKogo[]']").val()
  ...
  listInbox(folderId, page) {
    ...
    let listPromise = $ => {
      ...
      let parser = ($, row) => {
        ...
        return {
            id: $(children[2]).find("a").attr("href").match(/\d*$/)[0]
