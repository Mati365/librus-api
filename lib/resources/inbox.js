...
module.exports = class Inbox extends Resource {
  ...
  sendMessage(userIds, title, content) {
    ...
    // Use the working _request method with full URLs
    return this.api
      ._request("GET", "https://synergia.librus.pl/wiadomosci/2/5")
      .then(() => {
        // Prepare form data
        const formData = {};
        ...
        if (recipientIds.length === 1) {
          ...
          formData.DoKogo = recipientIds[0];
        } else {
          // Multiple recipients - use array format
          recipientIds.forEach((id, index) => {
            formData[`DoKogo[${index}]`] = id.toString();
          });
        }

        formData.temat = title;
        formData.tresc = content;
        formData.poprzednia = 6;
        formData.wyslij = "Wyślij";

        // Send message using _request with full URL
        return this.api
          ._request("POST", "https://synergia.librus.pl/wiadomosci/5", formData)
          .then((data) => {
            // Check for success message
            const $ = cheerio.load(data.html());
            const message = $(".green.container").text().trim();

            if (message && message.includes("została wysłana")) {
              return message;
            } else {
              throw new Error(
                "Message sending failed or no confirmation received",
              );
            }
          });
      });
  ...
  /**
   * Get recipient lists for all types including parents
   * https://synergia.librus.pl/wiadomosci/2/5
   *
  ...
  listReceivers(groupId = 0, isVirtualClasses = false) {
    ...
    // First, get the class ID for parents
    return this._getClassId().then((classId) => {
      console.log("Using class ID for parents:", classId);

      // Navigate to the page to get CSRF token
      return this.api._request("GET", "wiadomosci/2/5").then((data) => {
        // Extract CSRF token from the page
        const csrfTokenMatch = data
          .html()
          .match(/var csrfTokenValue = "([^"]+)"/);
        if (!csrfTokenMatch) {
        ...
        return this.api.cookie.getCookies(baseUrl).then((currentCookies) => {
          const dzienniksid = currentCookies.find(
            (c) => c.key === "DZIENNIKSID",
          );
          const sdzienniksid = currentCookies.find(
            (c) => c.key === "SDZIENNIKSID",
          );
          ...
          const requests = recipientTypes.map((recipientType) => {
            ...
            return axios
              ...
              .then((response) => {
                ...
                $("table.message-recipients-detail tr[class*='line']").each(
                  (index, row) => {
                    const $row = $(row);
                    const id = parseInt(
                      $row.find("input[name='DoKogo[]']").val(),
                    );
          ...
          // Add parents request if class ID is available
          if (classId) {
            const parentsRequest = (() => {
              const formData = new FormData();
              formData.append("typAdresata", "rodzic");
              formData.append("poprzednia", "5");
              formData.append("tabZaznaczonych", "");
              formData.append("czyWirtualneKlasy", isVirtualClasses.toString());
              formData.append("idGrupy", groupId.toString());
              formData.append("klasa_rada_rodzicow", classId.toString());
              ...
              return axios
                ...
                .then((response) => {
                  ...
                  return { type: "rodzic", recipients };
                });
            })();

            requests.push(parentsRequest);
          }
