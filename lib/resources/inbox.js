  ...
  getMessage(folderId, messageId) {
    ...
    return this.api._singleMapper(
      ...
      , ($, row) => {
        ...
        let attachment_names = $(row).find("img[src*=filetype]").map((idx, elem) => { return $(elem).parent().text().trim() });
        let attachment_paths = $(row).find("img[src*=download]").map((idx, elem) => { return $(elem).attr("onclick").match(/[^"]+wiadomosci[^"]+/)[0].replace(/\\/g, '').replace(/^\//, '') });
        let attachments = attachment_names.map((idx, elem) => {
          return {
            name: elem,
